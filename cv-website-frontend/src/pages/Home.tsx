...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-200 dark:bg-brand-dark p-0 sm:p-4 lg:p-10 transition-colors duration-500 font-sans">
      ...
      {/* CV CONTAINER */}
      <div className="max-w-[900px] mx-auto bg-white shadow-2xl flex flex-col md:flex-row min-h-[1272px] overflow-hidden cv-container cv-wrapper ring-1 ring-slate-200 dark:ring-brand-border print:shadow-none print:ring-0 print:m-0 print:w-full print:min-h-0">

        {/* SIDEBAR - LEFT */}
        <aside className="w-full md:w-[35%] bg-[#1a2b3c] text-white p-8 space-y-12 shrink-0 print:bg-[#1a2b3c] print:text-white print:w-[35%]">

          <div className="flex flex-col items-center space-y-6 pt-4">
            <div className="size-56 lg:size-64 rounded-full border-[6px] border-white/10 p-2 overflow-hidden bg-slate-700 shadow-2xl print:border-white/20 print:bg-slate-700">
              {cv.avatar ? (
                <img src={getImageUrl(cv.avatar)} alt={cv.fullName} className="w-full h-full object-cover rounded-full" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-white/20 print:text-white/20"><User size={90} /></div>
              )}
            </div>
            <div className="text-center space-y-2">
              <h1 className="text-4xl font-black tracking-tight uppercase leading-tight text-white print:text-white">{cv.fullName}</h1>
              <p className="text-lg text-slate-400 font-medium uppercase tracking-wider print:text-slate-300">{cv.jobTitle}</p>
            </div>
          </div>
          ...
          <section className="space-y-2 section">
            ...
            {cv.contact.linkedin && (
              <div className="flex items-center gap-4 text-sm font-medium text-white/90">
                <div className="size-8 bg-white/10 rounded-full flex items-center justify-center shrink-0"><LinkIcon size={14} /></div>
          ...
          <section className="space-y-4 section">
            <h2 className="text-xl font-black border-b-2 border-white/10 pb-2 uppercase tracking-wider text-white print:text-white print:border-white/20">{t.objective}</h2>
            <p className="text-[13px] leading-relaxed text-white/80 font-medium print:text-slate-100">{cv.introduction}</p>
          </section>

          <section className="space-y-4 section">
            <h2 className="text-xl font-black border-b-2 border-white/10 pb-2 uppercase tracking-wider text-white print:text-white print:border-white/20">{t.skills}</h2>
            <div className="space-y-4">
              {cv.skills.length > 0 ? cv.skills.map((skill, index) => (
                <div key={index} className="space-y-1.5">
                  <p className="text-xs font-bold text-white/90 uppercase tracking-wide print:text-white/90">{skill.name}</p>
                  <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden print:bg-white/10">
                    <div className="h-full bg-white/40 rounded-full print:bg-white/60" style={{ width: skill.level }} />
                  </div>
                </div>
          ...
          <section className="space-y-4 section">
            <h2 className="text-xl font-black border-b-2 border-white/10 pb-2 uppercase tracking-wider text-white print:text-white print:border-white/20">{t.interests}</h2>
            <ul className="grid grid-cols-2 gap-2 text-sm text-white/80 font-medium print:text-slate-100">
              {cv.interests ? cv.interests.split(',').map((interest, i) => (
                <li key={i} className="flex items-center gap-2 truncate"><span className="size-1.5 bg-white rounded-full print:bg-white" /> {interest.trim()}</li>
